...
( function() {
	CKEDITOR.plugins.add( 'mathjax', {
		...
		init: function( editor ) {
			console.log("mathjax plugin init");
			...
			editor.on( 'contentPreview', function( evt ) {
				evt.data.dataValue = evt.data.dataValue.replace(
					/<\/head>/, '<script src="' + CKEDITOR.getUrl( editor.config.mathJaxLib ) + '"><\/script><\/head>'
				);
			} );
	...
	CKEDITOR.plugins.mathjax.copyStyles = function( from, to ) {
		...
		for ( var i = 0; i < stylesToCopy.length; i++ ) {
			var key = stylesToCopy[ i ], val = from.getComputedStyle( key );
			if ( val )
				to.setStyle( key, val );
		}
	};
		...
		CKEDITOR.plugins.mathjax.frameWrapper = function( iFrame, editor ) {
			...
			function load() {
				...
				doc.write( '<!DOCTYPE html>' +
							...
							// Set the path to the Arabic extension for MathJax.
					
							// Load the Arabic extension for MathJax.
